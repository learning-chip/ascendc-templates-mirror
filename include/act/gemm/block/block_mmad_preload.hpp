...
#ifndef ACT_GEMM_BLOCK_BLOCK_MMAD_PRELOAD_HPP
#define ACT_GEMM_BLOCK_BLOCK_MMAD_PRELOAD_HPP
...
public:
    ...
    void operator()(
        ...
        AscendC::GlobalTensor<ElementB> const &gmNextBlockB,
        GemmCoord const &actualShape, GemmCoord const &actualShapeNext,
        bool isFirstBlock, bool hasNextBlock, float quantScale)
    {
        uint32_t mRound = RoundUp<L1AAlignHelper::M_ALIGNED>(actualShape.m());
        uint32_t nRound = RoundUp<L1BAlignHelper::N_ALIGNED>(actualShape.n());
        ...
        if constexpr (!ENABLE_UNIT_FLAG) {
            AscendC::SetFlag<AscendC::HardEvent::M_FIX>(EVENT_ID0);
            AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(EVENT_ID0);
            copyL0CToGm(quantScale, gmBlockC, l0CTensor, layoutBlock, layoutInL0C);
            AscendC::SetFlag<AscendC::HardEvent::FIX_M>(EVENT_ID0);
        } else {
            copyL0CToGm(quantScale, gmBlockC, l0CTensor, layoutBlock, layoutInL0C, 0b11);
        }
    }
...
#endif // ACT_GEMM_BLOCK_BLOCK_MMAD_PRELOAD_HPP
