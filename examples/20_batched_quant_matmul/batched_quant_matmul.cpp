...
struct Options {
    const std::string HELPER = "20_batched_quant_matmul b m n k [device_id]";
...
void Run(Options const &options)
{
    ...
    std::vector<int8_t> hostA(lenA, 1.0);
    golden::FillRandomData<int8_t>(hostA, DATA_LOWER_BOUND, DATA_UPPER_BOUND);
    uint8_t *deviceA{nullptr};
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void **>(&deviceA), sizeA, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMemcpy(deviceA, sizeA, hostA.data(), sizeA, ACL_MEMCPY_HOST_TO_DEVICE));

    // allocate memory of B and copy to device side
    std::vector<int8_t> hostB(lenB, 1.0);
    golden::FillRandomData<int8_t>(hostB, DATA_LOWER_BOUND, DATA_UPPER_BOUND);
    ...
    std::mt19937 gen(rd());
    std::uniform_real_distribution<float> dist(DATA_LOWER_BOUND, DATA_UPPER_BOUND);
    ...
    BatchedMatmul<<<aicCoreNum, nullptr, stream>>>(batchCount, problemShape,
        deviceA, layoutA, deviceB, layoutB, deviceC, layoutC, quantScale);
